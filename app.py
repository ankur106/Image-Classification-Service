...
from autoscale import RequestManager
...
def populate_to_sqs_request_queue():
    request_manager.increase_requests()
    cnt = 0
    output = None
    # print(request.files)
    if 'inputFile' in request.files:
        ...
        if f_name != '':
            ...
            try:
                ...
                output  = get_response(file_name)
                print("OUTPUT")
                print(output)
                data = {
                    "text" : output,
                }
                request_manager.requests_fulfilled()
                print(file_name + ':'+ output[0])
                return file_name+ ':'+ output[0]
